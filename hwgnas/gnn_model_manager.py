...
from hwgnas.gnn import GraphNet
from hwgnas.utils.model_utils import EarlyStop, TopAverage, process_action

import hwgnas.arch_process as g
...
class CitationGNNManager(object):
    ...
    def is_repreat_train(self, actions):
        # create model
        model = self.build_gnn(actions)
    ...
    # train from scratch
    def train(self, actions=None, format="two",skip_flag=0):
        ...
        path = self.args.log_output_dir + '/' + 'rand_seed' + str(self.args.random_seed) + '.txt'
        # path = self.args.log_output_dir + '.txt'

        end_time = time.time()
        # print("self.record_action_info(path, origin_action, val_acc, end_time - start_time)")
        # print("path=",path)
        # print("origin_action=",origin_action)
        # print("val_acc=",val_acc)
        if skip_flag==0:
            self.record_action_info(path, origin_action, val_acc, end_time - start_time)

        return reward, val_acc
        # return val_acc

    def record_action_info(self, path, origin_action, val_acc, time_cost):
        if origin_action[-1] != self.n_classes:
            origin_action[-1] = self.n_classes
            print('action to pred:', origin_action)
    ...
    def retrain(self, actions, format="two",skip_flag=0):
        return self.train(actions, format,skip_flag=skip_flag)

    def test_with_param(self, actions=None, format="two", with_retrain=False, skip_flag=0):
        return self.train(actions, format, skip_flag=skip_flag)
    ...
    def propose_gnnnet_mask(self, nums=1):
        net_mask = list( g.sampled_nets_generator(nums=nums) )

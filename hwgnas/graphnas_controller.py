...
class SimpleNASController(torch.nn.Module):
    ...
    def sample(self, batch_size=1, with_details=False):
        ...
        if with_details:
            return dags, torch.cat(log_probs), torch.cat(entropies), actions
    ...
    def get_logp_entropies(self, actions_index, batch_size=1):
        if batch_size < 1:
            raise Exception(f'Wrong batch_size: {batch_size} < 1')
        ...
        actions_index = torch.unsqueeze(actions_index, 1)
        for block_idx, action_name in enumerate(self.action_list):
            decoder_index = self.action_index(action_name)
            ...
            entropy = -(log_prob * probs).sum(1, keepdim=False)
            # action = probs.multinomial(num_samples=1).data
            action = torch.unsqueeze(actions_index[block_idx], 1)
        ...
        # actions = torch.stack(actions).transpose(0, 1)
        # print('actions_index:', actions_index)
